...
def img_to_tensor(file_path):
    ...
    """
    img = image.load_img(file_path, grayscale=True, target_size=(28, 28))
    x = image.img_to_array(img)
    x = np.expand_dims(x, axis=0)
    return x

import glob
a_list = glob.glob("data/A/*")
b_list = glob.glob("data/B/*")


a_tensor = np.vstack([img_to_tensor(x) for x in a_list])
b_tensor = np.vstack([img_to_tensor(x) for x in b_list])

X_train = np.vstack((a_tensor, b_tensor))
y_train = np.concatenate((np.zeros(a_tensor.shape[0]), np.ones(b_tensor.shape[0])))
